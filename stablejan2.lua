...
_G.ProgressionChat_Configuration = {
	...
	Themes = {
		-- paste urls like this: "https://raw.paste.com/theme.json"
		-- You can also paste paths to modules, this is for if you are making a theme of your own using studio, this makes it easier to test the theme. (WILL ONLY WORK ON STUDIO)
		-- the default theme will load on the first time you use progressionchat, to change to your custom themes, do so by opeing the settings interface and choosing your theme.
	},
	ShowTranslatedMessageDefault=true,-- show translated messages by default
	Default_Language="en", -- your default language
	Hook_Chat_Toggle=true, -- toggle weather progressionchat will hide / unhide when :SetCoreGuiEnabled("Chat") is invoked. (REQUIRES HOOKFUNCTION)
	...
	Show_Tool_Tips=true, -- toggle if tooltips are shown.
}
...
local Succ,Err = pcall(function()
    ...
    --/\/\/\/\/\ BRICKED FROM ROBLOX PATCH. Rewrite ASAP.
	...
	local developer_database = {} -- TEMPORARY!
	...
	function functions.playerChat(mesData)
		...
		if player then
			...
			-- autotranslate (forgot to impl)
			if (_G.ProgressionChat_Configuration["ShowTranslatedMessageDefault"]==true) then
				print("auto translated message")
				chatData.tag = {hasTag=true,text="[TL]",color=Color3.fromRGB(0, 0, 255)}
				chatData.translation = {messageTranslated=false,translatedMessage=""}
				chatData.message = translatedText
			end
